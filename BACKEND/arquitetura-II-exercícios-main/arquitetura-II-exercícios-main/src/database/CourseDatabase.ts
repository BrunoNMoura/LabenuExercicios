...
export class CourseDatabase extends BaseDatabase {
    ...
    public async updateCourseById(putCourseDB: CourseDB): Promise<void>{
        await CourseDatabase.connection(CourseDatabase.TABLE_COURSES).update(putCourseDB).where({id:putCourseDB.id})
    }
